...
pub fn run(project: &Project) -> Result<(), String> {
    ...
    // conta todos os arquivos antes de começar
    let total = count_files(&output_dir);
    logger::info(&format!("Enviando {} arquivo(s)...", total));

    let mut counter = 0usize;
    upload_dir(&mut stream, &output_dir, "/wwwroot", total, &mut counter)?;
...
fn upload_dir(
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            upload_dir(stream, &path, &name, total, counter)?;
            ...
            *counter += 1;
            ...
            stream
                ...
                .map_err(|e| {
                    logger::warn(&format!("[{}/{}] Falha ao enviar {}: {}", counter, total, name, e));
                    e.to_string()
                })
                .ok();

            logger::info(&format!("[{}/{}] {}/{}", counter, total, remote_dir, name));
        }
    }
